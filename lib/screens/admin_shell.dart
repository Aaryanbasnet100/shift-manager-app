...
class _AdminShellState extends State<AdminShell> {
  ...
  void _showAddEmpForm(BuildContext context) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: AppColors.surface, builder: (context) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 24, right: 24, top: 24), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(t('add_emp'), style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 24), buildNeonTextField(controller: nameCtrl, hint: t('name'), icon: Icons.badge_outlined), const SizedBox(height: 12), buildNeonTextField(controller: roleCtrl, hint: t('role'), icon: Icons.work_outline), const SizedBox(height: 12), buildNeonTextField(controller: userCtrl, hint: t('user'), icon: Icons.alternate_email), const SizedBox(height: 12), buildNeonTextField(controller: passCtrl, hint: t('pass'), icon: Icons.lock_outline), const SizedBox(height: 24), buildNeonButton(t('save_cloud'), () { if (nameCtrl.text.isNotEmpty && userCtrl.text.isNotEmpty) { FirebaseFirestore.instance.collection('restaurants').doc(widget.workspaceId).collection('employees').add({'restaurantId': widget.workspaceId, 'name': nameCtrl.text, 'role': roleCtrl.text.isEmpty ? 'Staff' : roleCtrl.text, 'username': userCtrl.text.toLowerCase(), 'password': passCtrl.text}); Navigator.pop(context); } }), const SizedBox(height: 40)])));
  }
